...
| Recency in Profile scripts | Profile scripts |  | <pre>// this code returns recency in days <br>// use 3600 * 1000 for hours and 60 * 1000 for minutes <br>var dayInMillis = 3600 * 24 * 1000;<br>if (!mbox.param('survey_topup_shown')){<br>  user.setLocal('lastSurveyTime', new Date().getTime());<br>}<br>var lastSurveyTime = user.getLocal('lastSurveyTime');<br>if (lastSurveyTime) {<br> return ((new Date()).getTime()-lastSurveyTime)/dayInMillis;<br>}</pre> |
